...
export default function AdminDashboard() {
  ...
  const linksByCategory = Array.isArray(links)
  ...
  const usersWithAssignments =
    assignments && Array.isArray(assignments)
      ? [...new Set(assignments.map((a) => a.userId))].length
      : 0;
  const linksWithAssignments =
    assignments && Array.isArray(assignments)
      ? [...new Set(assignments.map((a) => a.linkId))].length
      : 0;
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Manage your link management system with individual assignments
        </p>
      </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-semibold text-gray-900">
                {regularUsers.length > 0
                  ? (totalAssignments / regularUsers.length).toFixed(1)
                  : "0"}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600 mb-1">
              {regularUsers.length > 0
                ? Math.round((usersWithAssignments / regularUsers.length) * 100)
                : 0}
              %
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600 mb-1">
              {links && links.length > 0
                ? Math.round((linksWithAssignments / links.length) * 100)
                : 0}
              %
            </div>
        ...
        {totalAssignments === 0 && regularUsers.length > 0 && links && links.length > 0 && (
          <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex">
              ...
              <div className="ml-3">
                <h4 className="text-sm font-medium text-yellow-800">Ready to assign links!</h4>
                <p className="text-sm text-yellow-700 mt-1">
                  You have {regularUsers.length} users and {links.length} links ready. Start
                  assigning links to users to give them access.
                </p>
              </div>
            </div>
          </div>
